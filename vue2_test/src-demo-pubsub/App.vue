...
export default {
  ...
  methods: {
    ...
    editItem(id,val){
      this.list.forEach(l=>{
        if (l.id===id)l.title=val
      })
    }
  },
  components: {TopInput, MiddleList, FooterItem},
  mounted() {
    this.pubId = pubsub.subscribe('deleteItem', this.deleteItemPubsub)
  ...
  beforeDestroy() {
    ...
    this.$bus.$off(['deleteItem', 'changeItem','editItem'])
  }
}
</script>
<style>
